...
console.log(someWords);
let start = +prompt('Введите начальный индекс');
let end = +prompt('Введите начальный индекс');
for (i = start; i < end; i++) {
    if (end > someWords.length) {
        end = +prompt(`Введенное Вами число превышает длину массива. Введите число меньше или равное ${someWords.length}`);
    }
